    ...
    <div className="max-w-5xl mx-auto px-8 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-1">
          From theory to real-time apps — raw API, reconnection, live feeds, and
          Socket.io
        </p>
      </div>
      ...
      <div className="flex gap-8 items-start">
        ...
        <div className="flex-1 min-w-0">
          ...
          <ComingSoon
            ...
            desc="Request-reply pattern with timeout handling for reliable delivery."
          />
          <ComingSoon
            id="presence"
            badge="§11"
            title="Typing Indicators & Presence"
            desc="Debounced typing events, broadcast vs emit, online/offline state."
          />
          <AuthSection />
          <ComingSoon
            id="volatile"
            badge="§13"
            title="Volatile Events"
            desc="socket.volatile.emit() — when to drop events vs guarantee delivery."
          />
          <ComingSoon
            id="recovery"
            badge="§14"
            title="Connection State Recovery"
            desc="Socket.io v4.6+ buffering and replaying missed events on reconnect."
          />
          <ComingSoon
            id="ratelimit"
            badge="§15"
            title="Rate Limiting & Backpressure"
            desc="Defending against flooding; detecting and handling slow consumers."
          />
          <ComingSoon
            id="redis"
            badge="§16"
            title="Redis Adapter"
            desc="Scaling to multiple Node processes with @socket.io/redis-adapter."
          />
          <ComingSoon
            id="binary"
            badge="§17"
            title="Binary Data"
            desc="Sending ArrayBuffers and Blobs instead of JSON for performance."
          />
          <ComingSoon
            id="parser"
            badge="§18"
            title="Custom Parsers (msgpack)"
            desc="Replacing JSON with msgpack for smaller payloads and faster parsing."
          />
        </div>
      </div>
    </div>
