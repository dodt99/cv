...
const SlowItem = memo(function SlowItem({
  ...
  while (performance.now() - now < 0.08) {
    /* ~0.08ms × 8000 items = ~640ms */
  }
  ...
  return (
    <div>
      ...
      <ResultList
        ...
        label={
          mode === "with"
            ? "Results (deferred via startTransition)"
            : "Results (blocking)"
        }
      ...
      <div className="mt-4 rounded-xl border border-gray-200 overflow-hidden shadow-sm">
        ...
        <pre className="text-[12px] leading-relaxed font-mono bg-white p-4 overflow-x-auto text-gray-700">{`const [input, setInput] = useState("");
const [query, setQuery] = useState("");
const [isPending, startTransition] = useTransition();
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        >
          ...
          <p
            className={`text-sm font-mono min-h-[20px] ${
              isStale ? "text-amber-600" : "text-gray-800"
            }`}
          >
      ...
      >
        <ResultList
          query={deferredQuery}
          label={`Results using deferredQuery ${
            isStale ? "— showing previous results" : "— up to date"
          }`}
          isPending={isStale}
        />
      </div>
      ...
      <div className="mt-4 rounded-xl border border-gray-200 overflow-hidden shadow-sm">
        ...
        <pre className="text-[12px] leading-relaxed font-mono bg-white p-4 overflow-x-auto text-gray-700">{`const [query, setQuery] = useState("");
const deferredQuery = useDeferredValue(query);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 rounded-xl border-2 border-blue-100 bg-blue-50/30">
          <div className="flex items-center gap-2 mb-3">
            ...
            {isPending && (
              <span className="text-xs text-amber-600">updating…</span>
            )}
          </div>
          <ResultList
            query={transQuery}
            label="Deferred via startTransition"
            isPending={isPending}
          />
          <p className="text-xs text-gray-400 mt-2">
            You control which setState is deferred
          </p>
        </div>
        <div className="p-4 rounded-xl border-2 border-violet-100 bg-violet-50/30">
          ...
          <ResultList
            query={deferredInput}
            label="Deferred via useDeferredValue"
            isPending={input !== deferredInput}
          />
          <p className="text-xs text-gray-400 mt-2">
            You defer the value — useful when you don&apos;t own the setState
          </p>
        </div>
      </div>
    </div>
...
function ChallengeBox() {
  ...
  return (
    <div className="border-2 border-dashed border-amber-200 rounded-xl p-5 bg-amber-50/40">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-sm font-semibold text-amber-800 mb-1">
            Challenge: Tab switcher with heavy content
          </p>
          <p className="text-xs text-amber-700 leading-relaxed max-w-lg">
            Build a tab component with 3 tabs. One tab renders a heavy list of
            5000 items. Use{" "}
            <code className="bg-amber-100 px-1 rounded font-mono">
              useTransition
            </code>{" "}
            so the tab button responds instantly while the content loads, and
            show a loading state on the active tab.
          </p>
        </div>
        <button
      ...
      {show && (
        <pre className="mt-4 text-[11px] font-mono text-gray-700 bg-white border border-amber-200 rounded-lg p-3 overflow-x-auto leading-relaxed">{`const [tab, setTab] = useState("home");
const [isPending, startTransition] = useTransition();
  ...
  return (
    <div className="max-w-3xl mx-auto px-8 py-8">
      ...
      <div className="mt-4 flex items-start gap-2 text-xs text-gray-400">
        ...
        <p>
          The list renders <strong>8,000 items</strong> with an artificial
          ~0.08ms delay per item (~640ms total). This simulates real-world
          complex components like charts, rich text cells, or deeply nested
          trees. Open DevTools → Performance to see the difference in frame
          times.
        </p>
      </div>
    </div>
